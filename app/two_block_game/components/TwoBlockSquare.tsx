...
export default function TwoBlockSquare({ onSquareClick, occupied = true, target = false, targetId }: TwoBlockSquareProps) {
    return (
        <button onClick={onSquareClick} className="w-20 h-20 relative border-2 border-border bg-card hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed flex items-center justify-center rounded-xl transition-all duration-200 shadow-md hover:shadow-lg active:scale-95 group overflow-hidden">
            ...
            {occupied && target && (
                <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-amber-600 via-amber-700 to-amber-800 dark:from-amber-500 dark:via-amber-600 dark:to-amber-700 shadow-lg ring-2 ring-amber-900/20 dark:ring-amber-300/20 transition-transform duration-200 group-hover:scale-105 flex items-center justify-center">
                    {targetId === 'topLeft' && <span className="text-white font-bold text-xs">TL</span>}
                    {targetId === 'bottomRight' && <span className="text-white font-bold text-xs">BR</span>}
                </div>
            )}
            {!occupied && (
